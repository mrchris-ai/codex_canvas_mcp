...
Announcement-section updates require an exact course ID, announcement ID, expected title, and non-empty unique list of section IDs. The server verifies each section belongs to the approved course, sends only Canvas's documented `specific_sections` field, and reads back the exact section associations while protecting the announcement body, title, schedule, availability, publication state, and comment setting.
