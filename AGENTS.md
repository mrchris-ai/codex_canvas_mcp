...
- Writing is disabled when no policy is configured, when the policy is missing, or when the course is not allowlisted for the exact operation. The general content allowlist and rubric-deletion allowlist must remain separate.
- Permitted content-authoring and maintenance tools may create, update, or delete only Pages, Modules, module items, Assignments, assignment Rubrics, Discussions, and Classic Quizzes with their questions. Deletion tools must be resource-specific. Rubric deletion must require the exact course ID, rubric ID, and title; preserve the full definition before deletion; and refuse any rubric with Canvas-reported usage locations, foreign ownership, or read-only state. The sole local-payload exception is the image-only upload tool: it must remain limited to PNG, JPEG, and WebP files no larger than 10 MiB, rooted under `CANVAS_IMAGE_UPLOAD_ROOT`, and separately confirmed for the exact course. Do not add a generic write, arbitrary-method API tool, generic local-file upload tool, enrollment tool, or administrative API tool.
